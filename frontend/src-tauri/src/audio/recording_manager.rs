...
use super::bounded_queue::AudioQueueReceiver;
...
impl RecordingManager {
    ...
    pub async fn start_recording(
        ...
        auto_save: bool,
    ) -> Result<AudioQueueReceiver<AudioChunk>> {
        info!("Starting recording manager (auto_save: {})", auto_save);

        // Set up transcription channel
        let (transcription_sender, transcription_receiver) = self.pipeline_manager.create_transcription_queue();
    ...
    pub async fn start_recording_with_defaults_and_auto_save(&mut self, auto_save: bool) -> Result<AudioQueueReceiver<AudioChunk>> {
        #[cfg(target_os = "macos")]
        {
